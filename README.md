...
| feat        | 새로운 기능 추가 및 구현 |
| fix         | 버그 수정 및 오류 해결     |
| style       | 코드의 기능이나 의미 변화가 없는 스타일적인 수정에 사용     |
| refactor       | 기능 변경이나 버그 수정 없이, 코드의 구조, 가독성, 유지보수성, 성능 등을 개선했을 때 사용     |
| assets       | 정적인 에셋을 추가하거나 수정했을 때 사용     |
| chore       | 코드와 직접적인 관련이 없는 소소한 관리 작업    |
| merge       | 개발 브랜치(dev)로 다른 브랜치를 병합(Merge)했을 때 사용     |
